...
abstract contract KernelAdapter {
    KernelSol public kernel;

    constructor(KernelSol kernel_) {
        kernel = kernel_;
    }
    ...
    function changeKernel(KernelSol newKernel_) external onlyKernel {
        kernel = newKernel_;
    }
}
...
abstract contract Module is KernelAdapter {
    ...
    constructor(KernelSol kernel_) KernelAdapter(kernel_) {}
    ...
    function SETUP() external virtual onlyKernel {}
}
...
contract KernelSol {
    ...
    function executeAction(Actions action_, address target_) external onlyExecutor {
        if (action_ == Actions.InstallModule) {
            ...
            ensureContract(target_);
            _migrateKernel(KernelSol(target_));
    ...
    function _migrateKernel(KernelSol newKernel_) internal {
